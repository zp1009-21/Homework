...
#确定服务器所在信息
'''
1、登录按钮点击事件：当登录按钮点击时向服务端请求登录，如果登录成功则关闭登录页面，开启聊天页面。
2、注册按钮点击事件：当注册按钮点击时向服务端请求注册，得到回应后显示回应的消息
             （注册成功或注册失败、账号已存在等消息）。
3、刷新所有已登录用户列表。当开启聊天页面或收到服务端发来的新用户登录/登出的消息时刷新用户列表。
4、将聊天记录加入聊天记录显示框。当用户刚登录时显示世界聊天聊天记录
               当用户点击其他用户与其一对一聊天时显示与其的聊天记录。
5、当点击用户列表中的某用户时，显示与其一对一聊天的窗口。
6、接收服务端消息函数。该函数运行在一个独立的线程中，不断接收服务端发来的消息。
'''
server_ip = input("请输入服务器所在的主机ip：")
server_port = "8888"
